...
class Auth {
    ...
    return new Promise((resolve, reject) => {
      ...
      this._getUserPoolConfigInfo()
        .then(poolinfo => {
          ...
          Logger.log(
            Logger.levels.INFO,
            'Processing UI token for generation of user claim ticket.'
          );
          if (!pems) {
            ...
            request(
              ...
              function(error, response, body) {
                if (!error && response.statusCode === 200) {
                  ...
                  _self
                    ._validateToken(authorizationToken, poolinfo, iss)
                    .then(ticket => {
                      resolve(ticket);
                    })
                    .catch(err => {
                      ...
                      Logger.error(
                        1,
                        'Error occurred while validating authorization token to generate claim ticket.'
                      );
                      reject(err);
                    });
                } else {
        ...
        .catch(err => {
          ...
          Logger.error(
            Logger.levels.INFO,
            'Error occurred while attempting to retrieve user pool config info.'
          );
          reject(err);
        });
    });
  ...
  static _validateToken(authorizationToken, userPoolId, iss) {
    return new Promise((resolve, reject) => {
      ...
      if (!decodedJwt) {
        ...
        throw new Error(
          'Invalid JWT token decoded when validating authorization token.'
        );
      }
      ...
      if (decodedJwt.payload.iss != iss) {
        ...
        Logger.error(Logger.levels.INFO, JSON.stringify(_ticket));
        throw new Error(
          'Invalid issuer identified for decoded JWT token when validating authorization token.'
        );
      }
      ...
      if (decodedJwt.payload.token_use != 'id') {
        ...
        Logger.error(Logger.levels.INFO, JSON.stringify(_ticket));
        throw new Error(
          'Decoded token not identified as an Id Token when validating authorization token.'
        );
      }
      ...
      if (!pem) {
        ...
        Logger.error(Logger.levels.INFO, JSON.stringify(_ticket));
        throw new Error(
          'Invalid access token identified when validating authorization token.'
        );
      }
      ...
      jwt.verify(
        ...
        function(err, payload) {
          if (err) {
            ...
            throw new Error(
              'Unable to verify the signature of the JWT token to ensure it was generated from application User Pool'
            );
          } else {
            // Generate the claim ticket for the user.
            _self
              ._generateClaimTicket(
              ...
              .catch(err => {
                ...
                Logger.error(
                  1,
                  "Error occurred while attempting to generate the user's claim ticket."
                );
                throw err;
              });
          }
  ...
  static _generateClaimTicket(username, ticket, userPoolId) {
    return new Promise((resolve, reject) => {
      ...
      this._getUserFromCognito(username, userPoolId)
        .then(user => {
          ticket.auth_status = 'validated';
          ticket.auth_status_reason =
            'User is a valid entity of the user pool directory';
        ...
        .catch(err => {
          ...
          Logger.error(
            1,
            'Error occurred while attempting to retrieve user from Cognito.'
          );
          throw err;
        });
    });
    ...
    return new Promise((resolve, reject) => {
      ...
      cognitoidentityserviceprovider.adminGetUser(params, function(err, data) {
        if (err) {
          ...
          Logger.error(
            1,
            'Error occurred while attempting to retrieve user from user pool.'
          );
          throw err;
        }
        ...
        cognitoidentityserviceprovider.adminListGroupsForUser(params, function(
          ...
          if (err) {
            ...
            Logger.error(
              1,
              "Error occurred while attempting to retrieve user's groups from user pool."
            );
            throw err;
          }
